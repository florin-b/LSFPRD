...
public class CreareComanda extends Activity implements AsyncTaskListener, ValoareNegociataDialogListener, ComenziDAOListener,
				...
				logonHandler.post(new Runnable() {
					public void run() {
						...
						if ((dateLivrareInstance.getTipPlata().equals("E") || dateLivrareInstance.getTipPlata().equals("N") || dateLivrareInstance
								.getTipPlata().equals("R")) && CreareComanda.tipClientVar.equals("PJ")) {
		...
		if (totalComanda + Double.valueOf(totalNumerar) > 5000) {
			...
			builder.setMessage(
					"\nLa acest client valoarea comenzilor cu plata in numerar livrate in data de " + DateLivrare.getInstance().getDataLivrare()
							+ " depaseste 5000 de lei.\n\n" + "Pentru a salva comanda trebuie sa schimbati metoda de plata sau data de livrare.\n")
					.setCancelable(false).setPositiveButton("Inchide", new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int id) {
							dialog.cancel();
	...
	private void trateazaPretMacara(boolean acceptaPret, double valoarePret) {
		...
		if (acceptaPret) {
			...
			List<ArticolComanda> articoleDescarcare = HelperCostDescarcare.getArticoleDescarcareDistrib(costDescarcare, valoarePret,
					ListaArticoleComanda.getInstance().getListArticoleComanda());
	...
	private String prepareArtForDelivery() {
		...
		try {
			...
			List<ArticolComanda> listaArticole = ListaArticoleComanda.getInstance().getListArticoleComanda();
			List<ArticolTaxaVerde> listArticoleTVerde = new ArrayList<ArticolTaxaVerde>();
	...
	private void setLivrariMathaus(String result) {
		...
		for (ArticolComanda articolComanda : articoleComandaDistrib) {
			...
			if ((tipComandaDistributie.equals(TipCmdDistrib.COMANDA_LIVRARE) || UtilsComenzi.isComandaClp())
					&& articolComanda.getArticolMathaus() == null)
	...
	private void afisRezumatComandaDialog(List<CostTransportMathaus> costTransport, boolean selectTransp) {
		...
		RezumatComandaDialog rezumatComanda = new RezumatComandaDialog(this, ListaArticoleComanda.getInstance().getListArticoleComanda(), "10",
				costTransport, DateLivrare.getInstance().getTransport(), CreareComanda.filialeArondateMathaus, selectTransp);
	...
	private void adaugaPalet(ArticolPalet articolPalet, EnumPaleti status) {
		...
		String unitlogPalet = HelperCostDescarcare.getUnitlogPalet(ListaArticoleComanda.getInstance().getListArticoleComanda(),
				articolPalet.getCodArticol());
		...
		for (int ii = 0; ii < costDescarcare.getArticoleDescarcare().size(); ii++) {
			if (costDescarcare.getArticoleDescarcare().get(ii).getFiliala().equals(articolPalet.getFiliala())) {
				costDescarcare.getArticoleDescarcare().get(ii)
						.setCantitate(costDescarcare.getArticoleDescarcare().get(ii).getCantitate() + articol.getCantitate());
			}
		}
